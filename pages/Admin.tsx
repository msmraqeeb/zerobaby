      ...
      <aside className="w-72 bg-black text-white flex flex-col p-8 sticky top-0 h-screen shrink-0 shadow-2xl z-50">
        ...
        <nav className="flex flex-col gap-1 flex-1 overflow-y-auto custom-scrollbar">
          {[
            ...
            <button key={item.id} onClick={() => { setAdminTabState(item.id); closeForms(); }} className={`flex items-center gap-4 px-6 py-3.5 rounded-2xl transition-all font-bold text-sm ${adminTab === item.id ? 'bg-rose-50 text-[#004d40] shadow-lg' : 'text-slate-300 hover:bg-white/10'}`}>
